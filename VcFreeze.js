...
if (vcTemp.isTemp != true) {
  return lib.discord.channels['@0.3.2'].messages.create({
    channel_id: `${context.params.event.channel_id}`,
    content: `You cannot edit <#${authorVc}>, Only TempVCs are able to be Frozen!`,
    message_reference: { message_id: context.params.event.id, },
  });
}
...
if (vcTemp.owner != null && vcTemp.owner != `frozen` && vcTemp.owner != authorId) {
  return lib.discord.channels['@0.3.2'].messages.create({
    channel_id: `${context.params.event.channel_id}`,
    content: `You cannot freeze <#${authorVc}>. It belongs to \`${vcOwnerInfo.user.username}#${vcOwnerInfo.user.discriminator}\`, only they can freeze it!`,
    message_reference: { message_id: context.params.event.id, },
  });
}
...
if (vcTemp.owner == `frozen`) {
  return lib.discord.channels['@0.3.2'].messages.create({
    channel_id: `${context.params.event.channel_id}`,
    content: `<#${authorVc}> is already frozen.`,
    message_reference: { message_id: context.params.event.id, },
  });
}
...
await lib.discord.channels['@0.3.2'].messages.create({
  channel_id: `${context.params.event.channel_id}`,
  content: `<#${authorVc}> is now a frozen VC\n> This means it cannot be claimed nor edited.`,
  message_reference: { message_id: context.params.event.id, },
});
