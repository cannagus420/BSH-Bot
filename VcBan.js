...
if (mentionedUser) {
  ...
  await lib.discord.channels['@0.3.2'].messages.create({
    channel_id: `${context.params.event.channel_id}`,
    content: `\`${mentionedUser.username}#${mentionedUser.discriminator}\` has been banned from <#${authorVc}>!\n> Use \`/vc!unban @user\` to remove the ban`,
    message_reference: { message_id: context.params.event.id, },
  });
} else {
  await lib.discord.channels['@0.3.2'].messages.create({
