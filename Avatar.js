...
if (userInfo.avatar_url) {
  let extention = userInfo.avatar.startsWith('a_') ? 'gif' : 'png';
  let avatar_url = `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}.${extention}?size=4096`;
  await lib.discord.interactions['@1.0.0'].responses.create({
    ...
    embeds: [
      {
        type: 'rich',
        title: `Here is ${userInfo.username}'s Avatar`,
        description: ``,
  ...
  await lib.discord.interactions['@1.0.0'].responses.create({
    ...
    embeds: [
      {
        type: 'rich',
        title: `${userInfo.username} does not have an Avatar`,
        description: ``,
        color: 0xf32f2f, //red
        footer: {
          text: `Requested by ${context.params.event.member.user.username} • ${formattedTime}`,
